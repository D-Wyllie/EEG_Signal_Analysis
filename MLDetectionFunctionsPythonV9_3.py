...
def AnalyseSignalsFailsafeVer(StartVals,EndVals,signal,signalres2,signalCURVES,DFnorm,signalcurveave,gap):
    ...
    column_names_main = ['SignalNumber','SignalLength','NoPeaks','NoDips','NoPeaksBeforeGlobal','NoPeaksAfterGlobal','NoDipsBeforeGlobal','NoDipsAfterGlobal','DipDepth','MedianPeakDifference','Start','End']
    MainSignalDF = pd.DataFrame(columns = column_names_main, dtype=object)
...
#MAX LENGTH FOR END CROP IS 300000
    ...
    #Need it so it is more robust if difference is more than 1
        ...
        MainSignalDF.loc[K] = ['Signal' + str(K+1)]+[int(EndVals[K]) - int(StartVals[K])]+[len(maxtabx)]+[len(mintabx)]+[str(len(PeaksBeforeMainDip[0]))]+[str(len(mintabx) - len(PeaksBeforeMainDip[0]))]+[int(DipIndex)]+[int(len(mintaby) - DipIndex - 1)]+[np.min(mintaby)]+[np.median((DipRemoveMax+10000) - (DipRemoveMin+10000))]+[0]+[1]
...
#MAX LENGTH FOR END CROP IS 300000
    ...
    #Need it so it is more robust if difference is more than 1
        ...
        MainSignalDF.loc[K] = ['Signal' + str(signum+1) + '.' + str(K+1)]+[len(y_av_ultra_smooth)]+[len(maxtabx)]+[len(mintabx)]+[str(len(PeaksBeforeMainDip[0]))]+[str(len(mintabx) - len(PeaksBeforeMainDip[0]))]+[int(DipIndex)]+[int(len(mintaby) - DipIndex - 1)]+[np.min(mintaby)]+[np.median((DipRemoveMax+10000) - (DipRemoveMin+10000))]+[StartVals[K]]+[EndVals[K]]
